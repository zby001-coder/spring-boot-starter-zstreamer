...
public class UrlTool {
    ...
    public static String replacePlaceHolder(String in) {
        ...
        for (char c : in.toCharArray()) {
            if (HandlerConstance.PLACE_HOLDER_START == c) {
                inPlaceHolder = true;
                out.append(HandlerConstance.PLACE_HOLDER_REPLACER_STR);
                continue;
            } else if (HandlerConstance.PLACE_HOLDER_END == c) {
                inPlaceHolder = false;
                continue;
            }
            if (!inPlaceHolder) {
    ...
    public static String formatFilterPath(String in) {
        ...
        StringBuilder out = new StringBuilder(in.length());
        for (char c : in.toCharArray()) {
            if (c == HandlerConstance.PLACE_HOLDER_REPLACER_CHAR) {
                out.append(c);
                break;
            }
            out.append(c);
