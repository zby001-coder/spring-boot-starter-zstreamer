...
public class ContextHandler extends ChannelDuplexHandler {
    ...
    private final long httpMaxIdleTime;
    /**
     * 某一个channel当前请求的状态
     */
    ...
    public ContextHandler(Environment env) {
        httpMaxIdleTime = env.getProperty(ServerPropertyKeys.CONNECTION_MAX_IDLE_TIME, Long.class, ServerPropertyDefault.CONNECTION_MAX_IDLE_TIME);
    }
    ...
    public void write(ChannelHandlerContext ctx, Object msg, ChannelPromise promise) {
        ...
        ctx.write(msg, promise).addListener((future -> {
            ...
            expireTime = System.currentTimeMillis() + httpMaxIdleTime;
            inTransaction = false;
                ...
                expireTask = new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (!inTransaction && now >= expireTime) {
                            ...
                            ctx.channel().eventLoop().schedule(this, next > 0 ? next : httpMaxIdleTime, TimeUnit.MILLISECONDS);
                        }
                    }
                };
                ctx.channel().eventLoop().schedule(expireTask, httpMaxIdleTime, TimeUnit.MILLISECONDS);
            }
        }));
    }
